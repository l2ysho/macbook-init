...
- Copies custom scripts from [bin/](bin) into `~/bin` (currently `myip`, `psgrep`)
...
- Installs CLI tools and apps via `brew` (edit the `FORMULAE`/`CASKS` arrays in [init.sh](init.sh) to customize)
- Installs Mac App Store apps via `mas` (edit `MAS_APPS`) — requires you to already be signed into the App Store
- Applies macOS defaults: Finder (hidden files, extensions, path/status bar, list view, auto-empty Trash), keyboard (faster repeat, autocorrect off), Dock (icon magnification), trackpad (three-finger drag), screenshots saved to `~/Screenshots`
